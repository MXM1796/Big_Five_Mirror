...
- "score_calculation-layout": a directory representing a Python project that contains a "score_calculation"
  folder. Everything under the "score_calculation" folder is meant to be included in the
...
- "flat-layout": a Python project that does not use "score_calculation-layout" but instead
...
class ConfigDiscovery:
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        package_dir.pop("", None)  # This falls under the "score_calculation-layout" umbrella
    ...
    def _analyse_src_layout(self) -> bool:
        """Try to find all packages or modules under the ``score_calculation`` directory
        (or anything pointed by ``package_dir[""]``).

        The "score_calculation-layout" is relatively safe for automatic discovery.
        We assume that everything within is meant to be included in the
        distribution.

        If ``package_dir[""]`` is not given, but the ``score_calculation`` directory exists,
        this function will set ``package_dir[""] = "score_calculation"``.
        """
        package_dir = self._package_dir
        src_dir = os.path.join(self._root_dir, package_dir.get("", "score_calculation"))
        if not os.path.isdir(src_dir):
            return False

        log.debug(f"`score_calculation-layout` detected -- analysing {src_dir}")
    ...
    def _ensure_no_accidental_inclusion(self, detected: List[str], kind: str):
        if len(detected) > 1:
            ...
            2. use a `score_calculation-layout`
